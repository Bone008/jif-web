...
function main(args: string[]) {
  ...
  if (verbose) {
    ...
    for (const thrw of data.throws) {
      ...
      const fromStr =
        data.jugglers[fromLimb.juggler].label + ":" + fromLimb.label;
      const toLimb = data.limbs[thrw.to];
      const toStr = data.jugglers[toLimb.juggler].label + ":" + toLimb.label;
      console.log(
        `- t=${thrw.time}: ${thrw.duration} from ${fromStr} to ${toStr}`,
      );
    }
    console.log();
  }
    ...
    for (const interceptPerson of [0, 1, 2]) {
      for (const substitutePerson of [0, 1, 2]) {
        const spec: ManipulatorInstruction[] = [
          {
            type: "intercept2b",
            throwTime: interceptTime,
            throwFromJuggler: interceptPerson,
          },
          {
            type: "substitute",
            throwTime: (interceptTime + 4) % 6,
            throwFromJuggler: substitutePerson,
          },
        ];
  ...
  for (const [spec, jif, orbit] of goodResults) {
    console.log(
      ...
      orbit
        .map(
          (thrw) =>
            `${jif.jugglers[jif.limbs[thrw.from].juggler].label}${thrw.time}(${thrw.duration.toString(36)})`,
        )
        .join(" --> "),
    );
  }
